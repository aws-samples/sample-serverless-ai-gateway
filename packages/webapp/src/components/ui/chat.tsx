...
import { type ChatError } from "@/hooks/use-chat-state";
...
export function Chat({
    ...
    const handleStop = useCallback(() => {
        ...
        // Find the last assistant message (equivalent to findLast)
        ...
        if (lastAssistantMessage.toolInvocations) {
            const updatedToolInvocations =
                lastAssistantMessage.toolInvocations.map(
                    (toolInvocation: any) => {
    ...
    return (
        <ChatContainer className={className}>
            ...
            <ChatForm
                className="mt-4 sticky bottom-0 bg-background z-10 mb-4"
...
export function ChatMessages({
    ...
    return (
        <div
            className="grid grid-cols-1 overflow-y-auto pb-4 h-full min-h-0"
            ref={containerRef}
